...
handle_call(request_connection, _From, State) ->
  case State#state.connections of
	[] -> 
		...
		C;
	[C|H] ->
		NState = State#state{connections = H},
		C
	end,
  {reply, C, NState};

handle_call({release_connection, C}, _From, State) ->
  Connections = [C] ++ State#state.connections,
  NState = State#state{connections = Connections},
  {reply, ok, NState};
